...
export default function Inputrange({min, max, step, handler}) {
    ...
    const incrementHandler = () => {
        if(rangeRef.current){
            rangeRef.current.value = parseFloat(rangeRef.current.value) + parseFloat(step);
        }
        setValue(rangeRef.current.value);
        handler(rangeRef.current.value);
    }
    const decrementHandler = () => {
        if(rangeRef.current){
            rangeRef.current.value = parseFloat(rangeRef.current.value)>0? parseFloat(rangeRef.current.value) - parseFloat(step):0;
        }
        setValue(rangeRef.current.value);
        handler(rangeRef.current.value);
    }
